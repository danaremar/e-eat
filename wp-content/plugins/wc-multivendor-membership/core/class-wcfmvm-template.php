...
/**
 * @class 		WCFMvm Template Class
 *
 * @version		1.0.0
 * @package		WC
 * @author 		WC Lovers
 */
class WCFMvm_Template {

    public $template_url;

    public function __construct() {
        $this->template_url = 'wcfm/membership/';
    }
    ...
    public function get_template_part($slug, $name = '') {
        ...
        if (!$template && $name && file_exists($WCFMvm->plugin_path . "templates/{$slug}-{$name}.php"))
            $template = $WCFMvm->plugin_path . "templates/{$slug}-{$name}.php";
