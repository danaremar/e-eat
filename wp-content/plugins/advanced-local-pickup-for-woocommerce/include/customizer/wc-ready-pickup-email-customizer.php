 ...
 */
class wclp_ready_pickup_customizer_email {
	...
	public function __construct() {
		...
		if ( ! wclp_ready_pickup_customizer_email::is_own_customizer_request() && ! wclp_ready_pickup_customizer_email::is_own_preview_request() ) {
			return;
		}	
	...
	public function enqueue_preview_scripts() {		 
		...
		$preview_id     = get_theme_mod('wclp_ready_pickup_preview_order_id');
	...
	*/
	...
	public static function is_own_preview_request() {
		return isset( $_REQUEST['wclp-ready-pickup-email-customizer-preview'] ) && '1' === $_REQUEST['wclp-ready-pickup-email-customizer-preview'];
	}
	...
	public static function is_own_customizer_request() {
		return isset( $_REQUEST['email'] ) && $_REQUEST['email'] === 'customer_ready_pickup_email';
	}
	...
	public static function get_customizer_url($email) {		
		$customizer_url = add_query_arg( array(
			'wclp-customizer' => '1',
			'email' => $email,
			'url'                  => urlencode( add_query_arg( array( 'wclp-ready-pickup-email-customizer-preview' => '1' ), home_url( '/' ) ) ),
			'return'               => urlencode( wclp_ready_pickup_customizer_email::get_email_settings_page_url() ),
	...
	*/
	public function wclp_generate_defaults() {
		$customizer_defaults = array(			
			'wclp_ready_pickup_email_subject' => __( 'Your {site_title} order is now Ready for pickup', 'advanced-local-pickup-for-woocommerce' ),
			'wclp_ready_pickup_email_heading' => __( 'Your Order is Ready for pickup', 'advanced-local-pickup-for-woocommerce' ),
			'wclp_ready_pickup_email_content' => __( "Hi there. we thought you'd like to know that your recent order from {site_title} has been ready for pickup.", 'advanced-local-pickup-for-woocommerce' ),				
			'wclp_enable_ready_pickup_email'  => 'no',
			'location_box_border_color'  => '#ccc',
			'location_box_background_color' => '#fff',	
			'location_box_padding' => '10',
	...
	public function wclp_register_sample_default_controls( $wp_customize ) {		
		...
		*/
		...
		$wp_customize->add_setting( 'ready_pickup_order_email_heading',
			array(
				'default' => '',
		...
		$wp_customize->add_control( new WPLP_Customize_Heading_Control( $wp_customize, 'ready_pickup_order_email_heading',
			array(
				'label' => __( 'Ready for Pickup order status email', 'advanced-local-pickup-for-woocommerce' ),
				'description' => __( 'This section lets you customize the Email Content.', 'advanced-local-pickup-for-woocommerce' ),
				'section' => 'customer_ready_pickup_email'
			)
		) );							
		...
		$wp_customize->add_control( 'customizer_ready_pickup_order_settings_enabled',
			array(
				'label' => __( 'Enable Ready for Pickup order status email', 'advanced-local-pickup-for-woocommerce' ),
		...
		// Preview Order		
		$wp_customize->add_setting( 'wclp_ready_pickup_preview_order_id',
			array(
				'default' => 'mockup',
				'transport' => 'refresh',
				'sanitize_callback' => ''
		...
		$wp_customize->add_control( new WPLP_Skyrocket_Dropdown_Select_Custom_Control( $wp_customize, 'wclp_ready_pickup_preview_order_id',
			array(
				...
				'choices' => wclp_customizer()->get_order_ids(),
			)
		) );			
		...
		$wp_customize->add_control( 'woocommerce_customer_ready_pickup_order_settings[subject]',
			array(
				'label' => __( 'Subject', 'woocommerce' ),
				'description' => esc_html__( 'Available variables:', 'advanced-local-pickup-for-woocommerce' ).' {site_title}, {order_number}',
				'section' => 'customer_ready_pickup_email',
				'type' => 'text',
				'input_attrs' => array(
					'class' => '',
					'style' => '',
					'placeholder' => __( $this->defaults['wclp_ready_pickup_email_subject'], 'advanced-local-pickup-for-woocommerce' ),
				),
			)
		);
		...
		$wp_customize->add_control( 'woocommerce_customer_ready_pickup_order_settings[heading]',
			array(
				...
				'input_attrs' => array(
					'class' => '',
					'style' => '',
					'placeholder' => __( $this->defaults['wclp_ready_pickup_email_heading'], 'advanced-local-pickup-for-woocommerce' ),
				),
			)
		);
		
		// Test of TinyMCE control
		$wp_customize->add_setting( 'woocommerce_customer_ready_pickup_order_settings[additional_content]',
			array(
				'default' => $this->defaults['wclp_ready_pickup_email_content'],
				'transport' => 'refresh',				
				'type'  => 'option',
			)
		);
		$wp_customize->add_control( new WPLP_Skyrocket_TinyMCE_Custom_control( $wp_customize, 'woocommerce_customer_ready_pickup_order_settings[additional_content]',
			array(
				'label' => __( 'Email content', 'advanced-local-pickup-for-woocommerce' ),
				'description' => '',
				'section' => 'customer_ready_pickup_email',
				'input_attrs' => array(
					'toolbar1' => 'bold italic bullist numlist alignleft aligncenter alignright link',
					'mediaButtons' => true,
					'placeholder' => __( $this->defaults['wclp_ready_pickup_email_content'], 'advanced-local-pickup-for-woocommerce' ),
				)
			)
		) );	
		
		$wp_customize->add_setting( 'ready_for_pickup_email_code_block',
			array(
				'default' => '',
				'transport' => 'postMessage',
				'type'  => 'option',
		...
		$wp_customize->add_control( new WPLP_Customize_codeinfoblock_Control( $wp_customize, 'ready_for_pickup_email_code_block',
			array(
				'label' => __( 'Available variables:', 'advanced-local-pickup-for-woocommerce' ),
	...
	public function set_up_preview() {
		...
		if ( ! wclp_ready_pickup_customizer_email::is_own_preview_request() ) {
			return;
		}
		include wc_local_pickup()->get_plugin_path() . '/include/customizer/preview/ready_pickup_preview.php';		
		exit;			
	}
...
$wclp_ready_pickup_customizer_settings = new wclp_ready_pickup_customizer_email();
add_action( 'customize_save_customizer_ready_pickup_order_settings_enabled', 'woocommerce_customer_ready_pickup_order_settings_fun', 100, 1 ); 

/**
 * Update Delivered order email enable/disable
 *
 */
function woocommerce_customer_ready_pickup_order_settings_fun($data){
	$post_values = json_decode( wp_unslash( $_POST['customized'] ), true );
	$ready_pickup_order_settings = get_option( 'woocommerce_customer_ready_pickup_order_settings');
	
	if(isset($post_values['customizer_ready_pickup_order_settings_enabled']) && ($post_values['customizer_ready_pickup_order_settings_enabled'] == 1)){
		$ready_pickup_order_settings['enabled'] = 'yes';
	} else{
		$ready_pickup_order_settings['enabled'] = 'no';
	}		
	update_option( 'woocommerce_customer_ready_pickup_order_settings', wc_clean( $ready_pickup_order_settings ));	
}